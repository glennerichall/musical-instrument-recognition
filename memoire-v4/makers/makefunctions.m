...
% normalization.fun = normalizationfun;
normalization.minmax.train = @(data){min(data),max(data)};
normalization.musigma.train = @(data){mean(data),std(data)};
normalization.minmax.map = @(data,minmax) normalizationfun(data,(minmax{2}+minmax{1})/2,(minmax{2}-minmax{1})/2); % min-max [-1,1] normalization
normalization.musigma.map = @(data,musgima)normalizationfun(data,musgima{1},musgima{2});   % mu-sigma normalization
